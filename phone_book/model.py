...
def find_cotact(search_word: str)-> dict[int, list[str]]:
    global phone_book
    result  = {} 
...
def edit_contact(u_id: int, edit_contact: list[str]) -> str:
    global phone_book

    current_contact = phone_book[u_id]
    for i in range(len(current_contact)):
        current_contact[i] = edit_contact[i] if edit_contact [i] else current_contact
